def executeRegressionTests(heal_enabled, levels, browser, repo) {    
    ...
    node {
        ...
        stage('Execute Tests') {
            ...
            try {
                sh "docker run -v `pwd`/reports:/opt/robotframework/reports:Z -v `pwd`/project:/opt/robotframework/tests:Z -e BROWSER=$browser -e AWS_ACCESS_KEY_ID=AKIA2EMYZUE52BZL7B6A -e AWS_SECRET_ACCESS_KEY=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 -e levels=$levels -e healEnabled=$healValue healer"                       
            } catch (Exception e) {
